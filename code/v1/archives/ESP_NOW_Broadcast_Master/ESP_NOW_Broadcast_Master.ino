...
void loop() {
  ...
  char data[32];
  snprintf(data, sizeof(data), "Winston is SO fat", msg_count++);
After you pack X and Y into your struct — what do you need to do to actually send it via ESP-NOW?
  SerialAfter you pack X and Y into your struct — what do you need to do to actually send it via ESP-NOW?.printf("Broadcasting message: %s\n", data);
